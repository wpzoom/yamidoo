...
* Plays nicely with popular caching/optimization tools (WP Rocket, Cloudflare Rocket Loader).
...
* **What it is:** Yamidoo loads the chat widget script (`widget.js`) from the Yamidoo app and communicates with the Yamidoo API to fetch your widget's configuration and to power live conversations.
...
3. In WordPress, go to **Yamidoo** in the admin menu and paste the Site ID.
...
Yes. The widget script is marked so WP Rocket and Cloudflare Rocket Loader do not defer or combine it, which keeps the chat working reliably.
